	...
	implements vscode.CompletionItemProvider, IExtensionContribution
{
	...
	register(context: vscode.ExtensionContext): void {
		context.subscriptions.push(
			...
			registerCommand(
				vscode.commands,
				Commands.CompletionNodeTool,
				async (document, position) => {
					await new NodeToolInserter().selectAndInsertDebugConfig(
						document,
						position,
					);
				},
	...
	private async canProvideCompletions(
		...
		if (
			!(
				location.path[0] === "configurations" &&
		...
		const hasNodeModules =
			this.hasNodeModules.get(wf) ??
			!!(await existsInjected(
				fs,
...
class NodeToolInserter extends LaunchJsonUpdaterHelper {
	protected async getLaunchConfig(
		...
		return {
			...
			cwd: path
				.join("${workspaceFolder}", chosen.relativeDir)
				.replaceAll("\\", "/"),
			args: [],
		};
	}
