...
export interface IShutdownParticipants {
	...
	register(
		order: ShutdownOrder,
		p: (isFinal: boolean) => Promise<void>,
	): IDisposable;
...
export class ShutdownParticipants implements IShutdownParticipants {
	...
	async shutdownContext(): Promise<void> {
		if (
			this.shutdownStage === undefined ||
			this.shutdownStage < ShutdownOrder.ExecutionContexts
		) {
			await Promise.all(
				[...this.participants[ShutdownOrder.ExecutionContexts]].map(
					(p) => p(false),
				),
			);
		}
	...
	async shutdownAll(): Promise<void> {
		for (
			...
			await Promise.all(
				[...this.participants[this.shutdownStage]].map((p) => p(true)),
			);
		}
	}
}
