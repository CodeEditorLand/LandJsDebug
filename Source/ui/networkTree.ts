...
import {
	IMirroredNetworkEvents,
	mirroredNetworkEvents,
} from "../common/networkEvents";
	...
	implements IExtensionContribution, vscode.TreeDataProvider<NetworkNode>
{
	...
	constructor(
		...
		private readonly debugSessionTracker: DebugSessionTracker,
	) {
		this.disposables.push(
			...
			vscode.debug.onDidReceiveDebugSessionCustomEvent((event) => {
				if (event.event === "networkEvent") {
					this.models
						...
						?.append([event.body.event, event.body.data]);
				}
			}),
			this.debugSessionTracker.onSessionAdded((session) => {
				...
				session
					.customRequest("enableNetworking", {
						mirrorEvents: mirroredNetworkEvents,
					})
					.then(
						() => {
							this.models.set(
	...
	/** @inheritdoc */
	getChildren(
		element?: NetworkNode | undefined,
	): vscode.ProviderResult<NetworkNode[]> {
...
class FilesystemProvider implements vscode.FileSystemProvider {
	private readonly changeFileEmitter = new vscode.EventEmitter<
	...
	async stat(uri: vscode.Uri): Promise<vscode.FileStat> {
		...
		return {
			...
			ctime: request.ctime,
			mtime: request.mtime,
			size: request.isComplete
				? await request.body().then((b) => b?.length || 0)
				: 0,
		};
	}
	...
	async readFile(uri: vscode.Uri): Promise<Uint8Array> {
		...
		return Buffer.from(
			await request.toCurl(this.tracker.getById(sessionId)),
		);
	}
...
export class NetworkRequest {
	...
	public async toCurl(session: vscode.DebugSession | undefined) {
		...
		if (this.failed) {
			...
			if (this.failed.blockedReason) {
				...
				parts.push(
					`CORS error: ${this.failed.corsErrorStatus.corsError}`,
				);
			}
		}
	...
	public body = once(async () => {
		try {
			const res: Cdp.Network.GetResponseBodyResult =
				await this.session.customRequest("networkCall", {
