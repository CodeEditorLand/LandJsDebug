	...
	implements vscode.DebugConfigurationProvider
{
	protected async resolveDebugConfigurationAsync(
		...
		if (config.debugWebWorkerHost) {
			...
			config.resolveSourceMapLocations =
				extensionHostConfigDefaults.resolveSourceMapLocations;
		}
	...
	public async resolveDebugConfigurationWithSubstitutedVariables(
		...
		const config =
			debugConfiguration as ResolvingExtensionHostConfiguration;
...
const guessOutFiles = async (
	wf: vscode.WorkspaceFolder,
	pkgJson: IPackageJsonInfo,
) => {
	if (!pkgJson.value.main) {
		return undefined;
	}

	const extensionMain = path.resolve(
	...
	return [
		path
			.join("${workspaceFolder}", relativeToExt, subdirOfMain, "**/*.js")
			.replaceAll("\\", "/"),
	];
};
...
const resolveTestConfiguration = async (
	config: ResolvingExtensionHostConfiguration,
) => {
	...
	const result = await execa(
		process.execPath,
		[
			path.join(dirWithModules, suffix),
			"--config",
			testConfiguration,
			"--list-configuration",
		],
