...
const forSomeContextKeys = (
	...
	[...types]
		.map((d) => `${contextKey} == ${d}` + (andExpr ? ` && ${andExpr}` : ""))
		.join(" || ");
...
const forNodeDebugType = (contextKey: string, andExpr?: string) =>
	forSomeContextKeys(
		[DebugType.Node, DebugType.ExtensionHost, "node"],
		contextKey,
		andExpr,
	);
...
const baseConfigurationAttributes: ConfigurationAttributes<IBaseConfiguration> =
	{
		...
		timeouts: {
			...
			markdownDescription: refString(
				"timeouts.generalDescription.markdown",
			),
		},
		trace: {
	...
	configurationSnippets: [
		...
		},
		{
			...
			body: {
				...
				remoteRoot:
					"${3:Absolute path to the remote directory containing the program}",
				skipFiles: [`${nodeInternalsToken}/**`],
			},
		},
		{
...
const nodeLaunchConfig: IDebugger<INodeLaunchConfiguration> = {
	...
	configurationAttributes: {
		...
		runtimeVersion: {
			type: "string",
			markdownDescription: refString(
				"node.launch.runtimeVersion.description",
			),
			default: "default",
		},
		runtimeArgs: {
		...
		server: {
			oneOf: [
				...
				},
				{
					...
					properties:
						nodeTerminalConfiguration.configurationAttributes,
				},
			],
...
const extensionHostConfig: IDebugger<IExtensionHostLaunchConfiguration> = {
	...
	configurationAttributes: {
		...
		runtimeExecutable: {
			type: ["string", "null"],
			markdownDescription: refString(
				"extensionHost.launch.runtimeExecutable.description",
			),
			default: "node",
		},
		debugWebviews: {
		...
		rendererDebugOptions: {
			...
			properties: chromiumAttachConfigurationAttributes as {
				[key: string]: JSONSchema6;
			},
		},
		testConfiguration: {
			markdownDescription: refString(
				"extensionHost.launch.testConfiguration",
			),
			type: "string",
		...
		testConfigurationLabel: {
			markdownDescription: refString(
				"extensionHost.launch.testConfigurationLabel",
			),
			type: "string",
			default: "",
		},
		...
		for (const entry of entries) {
			entry.configurationAttributes[d.request] = {
				...
				properties: mapValues(
					d.configurationAttributes as {
						[key: string]: DescribedAttribute<unknown>;
					},
...
const configurationSchema: ConfigurationAttributes<IConfigurationTypes> = {
	...
	[Configuration.TerminalDebugConfig]: {
		...
		default: {},
		properties: nodeTerminalConfiguration.configurationAttributes as {
			[key: string]: JSONSchema6;
		},
	},
	...
	[Configuration.PickAndAttachDebugOptions]: {
		...
		properties: nodeAttachConfig.configurationAttributes as {
			[key: string]: JSONSchema6;
		},
	},
	...
	commandPalette: [
		...
		},
		{
			...
			when: forAnyDebugType(
				"debugType",
				"inDebugMode && !jsDebugIsProfiling",
			),
		},
		{
			command: Commands.StopProfile,
...
const activationEvents = new Set([
	...
	...[
		...debuggers.map((dbg) => dbg.type),
		...preferredDebugTypes.values(),
	].map((t) => `onDebugResolve:${t}`),
...
if (require.main === module) {
	process.stdout.write(
		JSON.stringify({
			...
			contributes: {
				menus,
				breakpoints: breakpointLanguages.map((language) => ({
					language,
				})),
