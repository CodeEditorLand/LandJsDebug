...
): definition is JSONSchema4 & { $ref: JSONSchema4 } {
...
async function generate() {
	...
	function generateType(prop: JSONSchema4): string {
		...
		if (prop.type === "array") {
			const subtype = prop.items
				? generateType(prop.items as JSONSchema4)
				: "any";
	...
	function appendProps(
		...
		for (const name in props) {
			...
			result.push(
				`${name}${requiredSet.has(name) ? "" : "?"}: ${generatedType};`,
			);
		}
	}
	...
	for (const name in defs) {
		...
		const desc = def.allOf.find((parent) => !hasRef(parent)) as
			| JSONSchema4
			| undefined;
		...
		if (ref.$ref === "#/definitions/Event") {
			...
			result.push(
				`${desc.properties!.event.enum![0]}(params: ${name}Params): void;`,
			);
			stubs.push({
				type: "event",
				name: `${name}Params`,
		...
		if (ref["$ref"] === "#/definitions/Request") {
			...
			const args = desc.properties?.arguments
				? desc.properties.arguments.$ref!
				: "#/definitions/";
			stubs.push({
				type: "params",
				name: `${title}Params`,
			...
			stubs.push({
				...
				name: `${title}Result`,
				value: defs[
					`${name.substring(0, name.length - "Request".length)}Response`
				],
			});

			appendText(desc.description);
			result.push(
				`${short}Request(params: ${title}Params): Promise<${title}Result>;`,
			);
		}
	}
	result.push(`}`);
	...
	for (const name in defs) {
		...
		if (ref["$ref"] === "#/definitions/Request") {
			...
			appendText(desc.description);
			result.push(
				`${short}(params: ${title}Params): Promise<${title}Result>;`,
			);
		}
	}
	result.push(`}`);
	...
	for (const type of stubs) {
		...
		if (type.type === "result") {
			...
			type.value = desc.properties
				? desc.properties.body
				: { properties: {} };
	...
	while (types.length) {
		...
		if (def.type && def.type !== "object") {
			...
			result.push(
				`export interface ${type} ${def.allOf ? getExtends(def) : ""} {`,
			);

			if (def.allOf) {
				// def extends some other interface(s)
				const ownDescription = def.allOf.find(
					(parent) => !hasRef(parent),
				)!;
				appendProps(
					ownDescription.properties!,
					ownDescription.required,
				);
			} else {
	...
	function generateTelemetryClassifications(definitions: {
		...
		const requestNames = definitionNames.filter((name) =>
			isRequest(definitions[name]),
		);

		const propertiesClassifications = requestNames.map((requestName) => {
			const noPostfixAndLowercase = requestName
				.replace(/Request$/, "")
				.toLowerCase();
