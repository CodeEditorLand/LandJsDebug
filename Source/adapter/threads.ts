...
const getReplSourceSuffix = () =>
	`\n//# sourceURL=eval-${randomBytes(4).toString(
		"hex",
...
export class Thread implements IVariableStoreLocationProvider {
	...
	private async _evaluteJs(
		...
		if (args.source && args.line && args.column) {
			...
			if (source && variables) {
				location = {
					source,
					position: new Base1Position(args.line, args.column),
					variables,
				};
			}
		}
	...
	async enableNetworking({
		...
		this._dap.with((dap) => {
			for (const event of mirrorEvents) {
				...
				(
					this._cdp.Network.on as (
						ev: string,
						fn: (d: object) => void,
					) => void
				)(event, (data) => dap.networkEvent({ data, event }));
			}
		});
	...
	async _executionContextsCleared() {
		...
		if (
			pausedDetails &&
			pausedDetails === this._pausedDetails &&
			!this.disposed
		) {
			this.onResumed();
		}
	...
	private async _onPaused(event: Cdp.Debugger.PausedEvent) {
		...
		const isWasmBreak =
			scriptId &&
			this._sourceContainer.getSourceScriptById(scriptId)?.url ===
		...
		if (isSourceMapPause) {
			...
			if (isWasmBreak) {
				...
				if (ids) {
					await Promise.all(
						ids.map((id) =>
							this._handleSourceMapPause(id, location),
						),
					);
				}
				return this.resume();
	...
	private _onScriptParsed(event: Cdp.Debugger.ScriptParsedEvent) {
		...
		if (
			existing &&
			this._executionContexts.has(existing.executionContextId)
		) {
			return;
		}
	...
	async _handleSourceMapPause(
		...
		const disableProm =
			this._breakpointManager.disableEntrypointBreaks(scriptId);
	...
	public async setScriptSourceMapHandler(
		...
		if (handler) {
			const installed = Promise.all(
				[...this._executionContexts.keys()].map((id) =>
					this._installWasmPauseHandler(id),
				),
			);
	...
	private _shouldEnablePerScriptSms(event: Cdp.Debugger.PausedEvent) {
		const script = this._sourceContainer.getScriptById(
			event.callFrames[0]?.location.scriptId,
		);
		if (!script) {
			return false;
		}
