...
class NodeVariable extends Variable {
	...
	public override async getChildren(
		params?: Dap.VariablesParamsExtended,
	): Promise<Variable[]> {
	...
	private async getValuePreview(_previewContext: PreviewContextType) {
		...
		const styleCheck = this.context.clientCapabilities.value
			?.supportsANSIStyling
			? true
			: "";
...
class FunctionVariable extends ObjectVariable {
	public override async toDap(
		...
		if (location) {
			dap.valueLocationReference =
				this.context.createFunctionLocationVariable(location).id;
		}
...
class WasmVariable implements IVariable, IMemoryReadable {
	...
	public async writeMemory(offset: number, memory: Buffer): Promise<number> {
		...
		const result = await this.context.cdp.Debugger.evaluateOnCallFrame({
			callFrameId: this.scopeRef.callFrameId,
			expression: `(${writeMemory.source}).call(memories[0].buffer, ${+addr + offset}, ${JSON.stringify(
				memory.toString("hex"),
			)}) ${getSourceSuffix()}`,
			returnByValue: true,
		});
...
export class VariableStore {
	...
	): Promise<Cdp.Debugger.Location> {
		const locationVar = this.vars.get(variablesReference);

		if (
			!locationVar ||
			!(locationVar instanceof FunctionLocationVariable)
		) {
