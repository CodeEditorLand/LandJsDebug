...
import {
	node15InternalsPrefix,
	nodeInternalsToken,
} from "../../common/node15Internal";
import {
	memoizeLast,
...
export class ScriptSkipper implements IScriptSkipper {
	...
	private _regexForAuthored = memoizeLast((re: readonly string[]) =>
		simpleGlobsToRe(re, (s) =>
			urlUtils.charRangeToUrlReGroup(s, 0, s.length, true, true),
		),
	);
		...
		if (nodeInternals) {
			...
			for (const internal of nodeInternals) {
				if (this._testSkipNodeInternal(internal)) {
					patterns.push(
						`^(${node15InternalsPrefix})?${escapeRegexSpecialChars(internal)}$`,
					);
				}
			}
		}
		...
		for (const authoredSource of source.sourceMap.sourceByUrl.values()) {
			...
			if (isSkippedSource !== inSkipRange) {
				...
				if (start && end) {
					skipRanges.push(
						{
							lineNumber: start.lineNumber - 1,
							columnNumber: start.columnNumber - 1,
						},
						{
